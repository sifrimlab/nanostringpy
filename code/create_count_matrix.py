...
def createCountMatrix(dcc_glob_pattern, pkc_file, meta_file):
    gene_dict = getGeneDict(pkc_file)
    metadata_df = createMetadataDf(meta_file)

    rows_list = []
    header = ["TargetName"]
    for file in glob(dcc_glob_pattern):
        gene_counts_dict = getCountDict(file, pkc_file, gene_dict = gene_dict)
        gene_counts_dict["TargetName"] =getPanelFromSampleID(os.path.splitext( os.path.basename(file))[0], metadata_df = metadata_df )
...
if __name__ == '__main__':
    createCountMatrix("./DCC/*.dcc", "./Mm_R_NGS_WTA_v1.0/Mm_R_NGS_WTA_v1.0.pkc", "./22Apr2021_MsWTA_20210427T2207_LabWorksheet.txt")
