...
count_mat <- count(pData(demoData), class, slide_name, region, segment)
nrow(pData(demoData))
count_mat
...
count_mat <- count(pData(target_demoData), class, slide_name, region, segment)
...
results <- c()
for(status in c("ChATpos", "ChATneg")) {
  ind <- pData(target_demoData)$segment == status
  mixedOutmc <-
...
kable(subset(results, Subset == "ChATpos"), digits = 3,
